...
Local_outlier <- function(group_data, result_common, Time) {
  ...
  for (i in seq_len(ncol(group_data))) {
    ...
    if (length(keep_idx) <= 2) {
      message("WARNING: Not enough points to refit curve ", i, " — skipping.")
      next
    }
    ...
    # --- PROTECTION: check if interp_clean has at least 2 non-NA values ---
    if (sum(!is.na(interp_clean)) < 2) {
      message("WARNING: interp_clean contains <2 non-NA values for curve ", i)
      next
    }

    fd_clean <- smooth.basis(grid, interp_clean, fdPar_obj)$fd

    # Evaluate cleaned curve
    clean_curve <- eval.fd(grid, fd_clean)

    # --- PROTECTION: check again for NA in clean_curve ---
    if (sum(!is.na(clean_curve)) < 2) {
      message("WARNING: clean_curve has <2 usable points for curve ", i)
      next
    }

    # --- SAFE interpolation (was causing the error) ---
    imputed_vals <- approx(grid, clean_curve, xout = Time[outliers], rule = 2)$y
