---
title: "plots velocity and acceleration"
output: html_document
date: "2025-11-21"
---


```{r}

rmarkdown::render("04_mean_and_variance_functions.Rmd", envir = globalenv(), quiet = TRUE)
rmarkdown::render("05_kinetics_derivatives.Rmd", envir = globalenv(), quiet = TRUE)

legends <- groups_names

ymin <- min(sapply(all_1st_derivs, min))
ymax <- max(sapply(all_1st_derivs, max))

```



```{r velocity }

if (!dir.exists("figures_1st_derivatives")) dir.create("figures_1st_derivatives")

for (i in seq_along(fd_groups)) {
  
  file_name <- paste0("figures_1st_derivatives/1st_derivative_", legends[i], ".png")
  png(file_name, width = 1400, height = 900, res = 150)
  
  
  plot(
    Time, all_1st_derivs[[i]], type = "l", col = colors[i], lwd = 3, lty = lty[i],
    xlab = "Time (sec)", ylab = "(fEPSP % baseline)'",
    main = paste("First Derivative -", legends[i]),
    ylim = c(ymin, ymax)   # <-- same scale
  )
  
  
  abline(h = 0, col = "red", lty = 1)
  grid()
  
  dev.off()
}




```



```{r velocity by group}

plot_1st_deriv <- function(mat, grid, filename, title, ylim,
                           colors, lty, legends) {
  
  png(filename, width = 1500, height = 1000, res = 150)
  
  par(mar = c(5, 4, 4, 10)) 
  plot(
    grid, mat[[1]],type = "n",xlab = "Time (sec)",ylab = "(fEPSP % baseline)'",
    main = title,ylim = ylim
  )
  
  
  for (i in seq_along(mat)) {
    lines(grid, mat[[i]],col = colors[i],lwd = 3,lty = lty[i]
    )
  }
  
  abline(h = 0, col = "red", lty = 1)
  grid()
  
  legend(
    "topright",inset=c(-0.27,0),xpd=TRUE, legend = legends,col = colors,lty = lty,lwd = 3,cex = 0.8,bty = "n"
  )
  
  dev.off()
}

subgroups <- list(
  Males_WT     = "^Males WT",Males_HRM    = "^Males HRM",
  Females_WT   = "^Females WT",Females_HRM  = "^Females HRM"
)

if (!dir.exists("figures_1st_derivatives_group")) {
  dir.create("figures_1st_derivatives_group")
}

for (sg in names(subgroups)) {
  
  idx <- grep(subgroups[[sg]], groups_names)
  
  filename <- paste0(
    "figures_1st_derivatives_group/1st_derivative_",
    tolower(sg), ".png"
  )
  
  plot_1st_deriv(
    mat      = all_1st_derivs[idx],grid     = Time,
    filename = filename,title    = paste("Mean First Derivative —", sg),
    ylim     = c(ymin, ymax),colors   = colors[idx],lty      = lty[idx],
    legends  = groups_names[idx]
  )
}




```




```{r}


early_idx <- Time >= 0 & Time <= 300


velocity_metrics_early <- function(deriv, time, idx) {
  
  deriv_e <- deriv[idx]
  time_e  <- time[idx]
  
  pos_idx <- deriv_e > 0
  
  data.frame(
    peak_velocity_early     = max(deriv_e, na.rm = TRUE),
    mean_positive_vel_early = mean(deriv_e[pos_idx], na.rm = TRUE)
  )
}



velocity_table_early <- do.call(
  rbind,
  lapply(seq_along(all_1st_derivs), function(i) {
    cbind(
      Group = groups_names[i],
      velocity_metrics_early(all_1st_derivs[[i]], Time, early_idx)
    )
  })
)

velocity_table_early[, -1] <- lapply(
  velocity_table_early[, -1], as.numeric
)

velocity_table_early[, -1]

```








```{r correlation of 1st derivative}

if (!dir.exists("matrices_cor")) dir.create("matrices_cor")

groups_list <- list(
  "Females WT " = grep("^Females WT", names(fd_groups), value = TRUE),
  "Males WT "   = grep("^Males WT", names(fd_groups), value = TRUE),
  "Females HRM " = grep("^Females HRM", names(fd_groups), value = TRUE),
  "Males HRM "   = grep("^Males HRM", names(fd_groups), value = TRUE)
)

for (grp_name in names(groups_list)) {
  group_fds <- fd_groups[groups_list[[grp_name]]]
  basis_fd <- group_fds[[1]]$basis
  
  coeff_list <- lapply(group_fds, function(fd) fd$coefs)
  coeff_mat <- do.call(cbind, coeff_list)
  
  group_fd <- fd(coef = coeff_mat, basisobj = basis_fd)
  deriv_gp <- deriv.fd(group_fd)
  
  cov_fd <- var.fd(deriv_gp)
  grid <- seq(basis_fd$rangeval[1], basis_fd$rangeval[2], length.out = 500)
  
  cov_matrix <- eval.bifd(grid, grid, cov_fd)

  # --- functional correlation ---
  v <- diag(cov_matrix)
  v[v == 0] <- 1e-10
  corr_matrix <- cov_matrix / sqrt( outer(v, v) )
  
  
  png(filename = paste0("matrices_cor/corr_heatmap_", grp_name, ".png"),
    width = 1200, height = 1000, res = 150)

  
  layout(matrix(c(1,2), ncol = 2), widths = c(4,1))

  par(mar = c(5,5,4,1))
  image(grid, grid, corr_matrix,
      col = topo.colors(50),
      zlim = c(-1, 1),
      xlab = "Time (s)", ylab = "Time (s)",
      main = paste("Functional correlation -", grp_name))

  
  par(mar = c(5,2,4,4))  # marges pour la légende
  image(1, seq(-1, 1, length.out = 50), 
      matrix(seq(-1, 1, length.out = 50), nrow = 1),
      col = topo.colors(50),
      axes = FALSE,
      xlab = "", ylab = "")
  axis(4, at = seq(-1, 1, by = 0.5), las = 1)  # axe y à droite

  dev.off()

}

```









