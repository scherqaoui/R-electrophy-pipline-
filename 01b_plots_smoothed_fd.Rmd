...
plot_boxplot <- function(fd_gp, title_gp, col, lty,
  ...
  grid <- seq(min(Time), max(Time), length.out = 200)
  mat  <- eval.fd(grid, fd_gp)   # fbplot accept only matrices evaluated on grid
  
  fbplot(mat, method = 'MBD', ylim = ylim_global)
  
  title(main = title_gp, xlab = "Time (sec)", ylab = "fEPSP % of baseline")
  
  legend("topright",
         legend = c("envelopes.", "median curve", "outlier candidates"),
         col = c("#0072B2", "black", "red"),
...
#The functional boxplot of SST with blue curves denoting envelopes, and a black curve representing the median curve. The red dashed curves are the outlier candidates detected by the 1.5 times the 50% central region rule.  
