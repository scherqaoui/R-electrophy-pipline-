...
WT_idx <- grep(" WT ", groups_names)
mean_values_WT <- mean_values_list[WT_idx]


groups_WT_names <- groups_names[WT_idx]


colors_WT <- colors[WT_idx]
lty_WT    <- lty[WT_idx]
...
plot(grid, mean_values_WT[[1]], type="n", ylim=ylim_mean,
     xlab="Time (sec)", ylab="fEPSP % of baseline",
     main="Mean function curve — WT")

for (i in seq_along(mean_values_WT)) {
  lines(grid, mean_values_WT[[i]], col=colors_WT[i], lty=lty_WT[i], lwd=2)
}

grid()
legend("topright", inset=c(-0.27,0), xpd=TRUE,
       legend=groups_WT_names, col=colors_WT, lty=lty_WT, lwd=2, bty="n", cex=0.8)
...
HRM_idx <- grep(" HRM ", groups_names)
mean_values_HRM <- mean_values_list[HRM_idx]


groups_HRM_names <- groups_names[HRM_idx]


colors_HRM <- colors[HRM_idx]
lty_HRM    <- lty[HRM_idx]
...
plot(grid, mean_values_HRM[[1]], type="n", ylim=ylim_mean,
     xlab="Time (sec)", ylab="fEPSP % of baseline",
     main="Mean function curve — HRM")

for (i in seq_along(mean_values_HRM)) {
  lines(grid, mean_values_HRM[[i]], col=colors_HRM[i], lty=lty_HRM[i], lwd=2)
}

grid()
legend("topright", inset=c(-0.27,0), xpd=TRUE,
       legend=groups_HRM_names, col=colors_HRM, lty=lty_HRM, lwd=2, bty="n", cex=0.8)
